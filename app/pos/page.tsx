...
export default function POSPage() {
  ...
  const posModalOpen = showNewProductModal || showScanner || showHistoryModal || showCustomerSearch || showCustomerCreateModal || showInvoiceModal || checkoutStep !== "cart";
  ...
  const handleSearchInvoices = async () => {
    ...
    try {
      const res = await invoicesApi.list("partial,sent,draft,overdue");
      let invoices = res.invoices || [];
      if (invoiceSearchPhone.trim()) {
        invoices = invoices.filter((inv: any) =>
          inv.clientPhone?.includes(invoiceSearchPhone.trim()) ||
          inv.clientName?.toLowerCase().includes(invoiceSearchPhone.trim().toLowerCase())
        );
      }
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setInvoiceSearchPhone("");
                      setSelectedInvoice(null);
                      setInvoicePaymentAmount("");
                      setUnpaidInvoices([]);
                    }}

                    className="h-7 px-2.5 text-[11px] font-medium text-indigo-600 hover:bg-indigo-50 rounded-lg flex items-center gap-1 transition-colors border border-indigo-200 shrink-0"
                      ...
                      {unpaidInvoices.map((inv) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setInvoicePaymentAmount(String(inv.balance));
                          }}
                          className={cn(
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-xs text-[var(--text-muted)]">{inv.clientName} · {inv.clientPhone}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-sm font-bold tabular-nums">{formatCurrency(inv.balance)}</p>
                              <p className="text-[10px] text-[var(--text-muted)] uppercase">{inv.status}</p>
                            </div>
                          </div>
                        </button>
                      ))}
