...
export default function ReturnsPage() {
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      ...
      toast("Error loading transactions", "warning");
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  const handleSubmit = async () => {
    if (!selectedTx) {
      toast("Select a transaction", "warning");
      return;
    }
    ...
    if (!reason) {
      toast("Enter a return reason", "warning");
      return;
    }
    ...
    try {
      ...
      toast("Return saved successfully", "success");
      // Reset
      setSelectedTx(null);
      setReturnItems([]);
      setReason("");
      ...
      toast(e?.message || "Error saving return", "warning");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="flex flex-col h-screen bg-[#f0f2f5] overflow-hidden">
      ...
      <div className="h-11 bg-white border-b border-[#e5e7eb] flex items-center px-5 gap-4 shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-bold text-[#111827]">Product Returns</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[40%] flex flex-col bg-white border-r border-[#e5e7eb] shrink-0">
          <div className="p-4 border-b border-[#e5e7eb]">
            <h2 className="text-sm font-bold text-[#111827] mb-3">1. Search transaction</h2>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-4 text-center text-sm text-[#9ca3af]">Loading...</div>
            ) : filteredTx.length === 0 ? (
              <div className="p-4 text-center text-sm text-[#9ca3af]">No transactions</div>
            ) : (
              filteredTx.map((tx) => (
                <button
                  key={tx.id}
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-[#9ca3af]">{new Date(tx.date).toLocaleString("en-GB")}</p>
                      <p className="text-xs text-[#6b7280] mt-0.5">{tx.items?.length || 0} item(s)</p>
                    </div>
          ...
          {!selectedTx ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-[#9ca3af]">Select a transaction on the left</p>
              </div>
            </div>
          ) : (
            <>
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                <div className="bg-white rounded-xl p-4 border border-[#e5e7eb]">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p className="text-xs text-[#9ca3af]">Original total</p>
                ...
                {/* Items to return */}
                <div className="bg-white rounded-xl p-4 border border-[#e5e7eb]">
                  <h3 className="text-sm font-bold text-[#111827] mb-3">2. Items to return</h3>
                  {returnItems.length === 0 ? (
                    <p className="text-sm text-[#9ca3af]">No items in this transaction</p>
                  ) : (
                      ...
                      {returnItems.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-3 p-2 rounded-lg bg-[#f9fafb]">
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-[#9ca3af]">{formatCurrency(item.unitPrice)} / unit</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-[#9ca3af]">Max qty: {txItemMaxQty(idx)}</span>
                            <input
                              type="number"
                              min={0}
                  ...
                  {totalRefund > 0 && (
                    <div className="mt-3 pt-3 border-t border-[#e5e7eb] flex justify-between">
                      <span className="text-sm font-bold text-[#111827]">Total to refund</span>
                ...
                <div className="bg-white rounded-xl p-4 border border-[#e5e7eb] space-y-3">
                  <h3 className="text-sm font-bold text-[#111827]">3. Return details</h3>

                  <div>
                    <label className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">Customer (optional)</label>
                    <input
                      type="text"
                      value={clientName}
                  ...
                  <div>
                    <label className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">Return reason</label>
                    <input
                      type="text"
                      value={reason}
                      onChange={(e) => setReason(e.target.value)}
                      placeholder="e.g. Defective product, error..."
                      className="w-full mt-1 px-3 py-2 border border-[#e5e7eb] rounded-lg text-sm outline-none focus:border-[#f97316]"
                    />
                  </div>

                  <div>
                    <label className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">Resolution</label>
                    <div className="grid grid-cols-3 gap-2 mt-1">
                      {(["refund", "exchange", "store_credit"] as const).map((r) => (
                        ...
                        >
                          {r === "refund" ? "Refund" : r === "exchange" ? "Exchange" : "Store Credit"}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {resolution === "refund" && (
                    <div>
                      <label className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">Refund method</label>
                      <select
                        value={refundMethod}
              ...
              <div className="p-4 border-t border-[#e5e7eb] bg-white">
                {(totalRefund === 0 || !reason) && (
                  <p className="text-xs text-amber-600 mb-2 text-center">
                    {!reason ? "Enter a return reason to enable validation" : "Select at least one item to return"}
                  </p>
                )}
                <Button
                ...
                >
                  {submitting ? "Saving..." : `Validate return (${formatCurrency(totalRefund)})`}
                </Button>
              </div>
            </>
          )}
